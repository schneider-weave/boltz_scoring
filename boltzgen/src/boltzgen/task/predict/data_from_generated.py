...
class FromGeneratedDataset(torch.utils.data.Dataset):
    ...
    def getitem_from_paths(self, metadata_path, generated_path, native_path) -> Dict:
        ...
        # get conditioning information from metadata - deactivated for nova
        ...
        design_mask = None #design chain determined by CIF chain ID
    ...
    def get_feat(self, path, design_mask=None, ss_type=None, binding_type=None):
        ...
        # NOVA PATCH: FORCE DESIGN CHAIN TO BE CHAIN "B"
        if design_mask is None:
            asym_id = tokenized.tokens["asym_id"] # token-aligned
            design_mask = (asym_id == 1).astype(np.float32)

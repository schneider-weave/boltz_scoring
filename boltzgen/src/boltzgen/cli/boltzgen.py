...
def build_configure_parser(subparsers) -> argparse.ArgumentParser:
    ...
    group.add_argument(
        ...
        nargs="+",
        type=Path,
        help="Path(s) to design specification YAML file(s) or a directory containing design specification YAML files",
    )
...
def configure_command(args: argparse.Namespace) -> None:
    ...
    * `<output_dir>/steps.yaml` — manifest of steps and config file paths.
    ...
    Usually this is executed by `boltzgen run ...` but it can be used like:
        ...
        $ boltzgen configure path/to/configs/ --output out_dir --protocol peptide-anything
    """
    args.design_spec = expand_design_specs(args.design_spec)
...
### Misc utiltiies ###
def expand_design_specs(design_specs: List[Path]) -> List[Path]:
    """
    Expand design specification paths to include all YAML files.
    
    If a path is a directory, recursively find all .yaml and .yml files within it.
    If a path is a file, include it as-is.
    
    Parameters
    ----------
    design_specs : List[Path]
        List of paths that may be files or directories
        
    Returns
    -------
    List[Path]
        Expanded list of YAML file paths
    """
    expanded = []
    for spec in design_specs:
        spec = Path(spec).resolve()
        if spec.is_dir():
            yaml_files = sorted(spec.rglob("*.yaml")) + sorted(spec.rglob("*.yml"))
            if not yaml_files:
                return None
...
def get_artifact_path(
    args, artifact: str, repo_type: str = "model", verbose: bool = False
) -> Path:
